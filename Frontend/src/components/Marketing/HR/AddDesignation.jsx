...
import { Plus, Briefcase, Trash2, Edit2, X, Check, Download } from "lucide-react";
...
const AddDesignation = () => {
  ...
  const filteredDesignations = designations.filter(
    (designation) =>
      ...
      (designation.id && designation.id.toString().includes(searchTerm))
  );

  const exportToPDF = () => {
    try {
      const doc = new jsPDF();
      const title = "Designations Report";
      ...
      if (searchTerm.trim()) {
        doc.setFontSize(9);
        doc.text(`Search Filter: "${searchTerm}"`, 14, 29);
        doc.text(`Total Designations: ${filteredDesignations.length}`, 14, 35);
      } else {
        doc.setFontSize(9);
        doc.text(`Total Designations: ${filteredDesignations.length}`, 14, 29);
      }

      // Prepare table data
      const tableData = filteredDesignations.map((designation, index) => [
        index + 1,
        designation.designation || '-',
        formatDate(designation.created_date) || '-',
        formatDate(designation.updated_date) || '-'
      ]);

      // Add table
      doc.autoTable({
        head: [['S.NO', 'Designation', 'Created Date', 'Updated Date']],
        body: tableData,
        startY: searchTerm.trim() ? 42 : 36,
        theme: 'grid',
        styles: {
          fontSize: 9,
          cellPadding: 3,
      ...
      // Save PDF
      const filename = `Designations_${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);

      notify({
        title: "Success",
        message: `Exported ${filteredDesignations.length} designation(s) to PDF`,
      });
    } catch (error) {
        ...
        <div className="flex items-center gap-[0.5vw]">
          <div className="relative">
            <img
              src={SearchIcon}
              alt=""
              className="w-[1.1vw] h-[1.1vw] absolute left-[0.6vw] top-1/2 transform -translate-y-1/2 text-gray-400"
            />
            <input
              type="text"
              ...
              className="pl-[2.1vw] pr-[0.6vw] py-[0.35vw] rounded-full text-[0.85vw] bg-gray-100 border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          <button
            onClick={exportToPDF}
            disabled={filteredDesignations.length === 0}
            className="px-[0.8vw] py-[0.35vw] bg-green-600 text-white rounded-full hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-[0.78vw] flex items-center justify-center cursor-pointer transition whitespace-nowrap"
            title={filteredDesignations.length === 0 ? "No designations to export" : `Export all ${filteredDesignations.length} filtered designation(s) to PDF`}
          >
            <Download className="w-[0.85vw] h-[0.85vw] mr-[0.3vw]" />
            Export PDF
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col min-h-0">
        <div className="bg-gray-900 text-white px-[0.9vw] py-[0.55vw] flex items-center justify-between">
          ...
          <span className="text-[0.72vw] text-gray-300">
            {searchTerm.trim() ? `Filtered: ${filteredDesignations.length}` : `Total: ${filteredDesignations.length}`}
          </span>
        </div>
        ...
        <div className="flex-1 overflow-auto divide-y divide-gray-200">
          {loading ? (
            ...
            <div className="px-[0.9vw] py-[2vw] text-center text-gray-500 text-[0.8vw]">
              {searchTerm.trim() 
                ? `No designations found matching "${searchTerm}"`
