...
router.post("/leave-requests", async (req, res) => {
  try {
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')`,
    ...
    const requesterDesignation = (userData.designation || "").toLowerCase();
    const isRequesterPHorSBU = requesterDesignation.includes("project head") || requesterDesignation.includes("sbu");
    ...
    // Send notifications:
    // - If requester is PH/SBU: only Admin gets notified (not HR, not other PH/SBU)
    // - Otherwise: Admin, HR, PH, SBU all get notified
    const employeeIds = targetEmployees
      .filter((emp) => {
        if (emp.employee_id === employee_id) return false; // Never notify self
        const empDesig = (emp.designation || '').toLowerCase();
        if (isRequesterPHorSBU) {
          return empDesig.includes('admin'); // Only Admin receives notification for PH/SBU requests
        }
        return true;
      })
