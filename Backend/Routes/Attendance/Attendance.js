...
// POST - Manually create/update full attendance record
router.post("/manual", async (req, res) => {
  ...
  try {
    ...
    if (!employee_id || !employee_name || !login_date) {
    ...
    if (existing.length > 0 && !overwrite) {
      return res.status(409).json({
        status: false,
        message: "A record for this date already exists. Do you want to overwrite it?",
        requiresConfirmation: true
      });
    }

    // Format times properly - ensure they are in HH:MM:SS format if provided
    const formatTime = (t) => {
      if (!t) return null;
    ...
    const m_in = formatTime(morning_in);
    const m_out = formatTime(morning_out);
    const a_in = formatTime(afternoon_in);
    const a_out = formatTime(afternoon_out);

    const totalHours = formatDuration(m_in, m_out, a_in, a_out);
    const status = calculateStatus(m_in, m_out, a_in, a_out);

    if (existing.length > 0) {
      await new Promise((resolve, reject) => {
        connection.query(
          `UPDATE attendance SET morning_in = ?, morning_out = ?, afternoon_in = ?, afternoon_out = ?, total_hours = ?, status = ? WHERE employee_id = ? AND login_date = ?`,
          [m_in, m_out, a_in, a_out, totalHours, status, employee_id, login_date],
          (err) => (err ? reject(err) : resolve())
        );
      });
    } else {
      await new Promise((resolve, reject) => {
        connection.query(
          `INSERT INTO attendance (employee_id, employee_name, login_date, morning_in, morning_out, afternoon_in, afternoon_out, total_hours, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [employee_id, employee_name, login_date, m_in, m_out, a_in, a_out, totalHours, status],
          (err) => (err ? reject(err) : resolve())
        );
      });
    }
    ...
    res.json({
      status: true,
      message: existing.length > 0 ? "Attendance updated successfully" : "Attendance created successfully"
    });
  } catch (error) {
