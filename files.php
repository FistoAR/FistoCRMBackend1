...
if (isset($_POST['_login'])) {
    if (trim($_POST['username'] ?? '') === DEMO_USER &&
...
if (isset($_GET['api']) && $loggedIn) {
    ...
    function jOk(mixed $d = null): never  { echo json_encode(['ok'=>true,'d'=>$d]); exit; }
    function jErr(string $m, int $c=400): never {
        http_response_code($c); echo json_encode(['ok'=>false,'e'=>$m]); exit;
    }
    ...
    function sp(string $rel): string {
        ...
        if (strpos($r, ROOT_DIR) !== 0) jErr('Access denied');
        return $r;
    }
    ...
    function fCat(string $name): string {
        ...
        ] as $cat=>$exts) if(in_array($e,$exts,true)) return $cat;
        return 'file';
    }
    function fi(string $abs): array {
        ...
        return['name'=>$name,'path'=>rel($abs),'isDir'=>$isDir,'ext'=>$ext,
               'size'=>$sz,'sizeH'=>fmtSz($sz),'mod'=>filemtime($abs),
               'cat'=>$isDir?'folder':fCat($name),'w'=>is_writable($abs)];
    }
    function rmRec(string $dir): bool {
        $e=@scandir($dir); if(!$e) return false;
        foreach($e as $f){ if($f==='.'||$f==='..') continue;
            $p=$dir.DIRECTORY_SEPARATOR.$f; is_dir($p)?rmRec($p):@unlink($p); }
        return @rmdir($dir);
    }
    ...
    try {
        ...
        case 'list':
            ...
            if(!is_dir($dir)) jErr('Not a directory');
            $entries=@scandir($dir); if(!$entries) jErr('Cannot read');
            $items=[];
            foreach($entries as $e){ if($e==='.'||$e==='..') continue; $items[]=fi($dir.DIRECTORY_SEPARATOR.$e); }
        ...
        case 'tree':
            function tree(string $dir, int $d=0): array {
                if($d>6) return [];
                $e=@scandir($dir); if(!$e) return [];
                $out=[];
                foreach($e as $f){ if($f==='.'||$f==='..') continue;
                    $a=$dir.DIRECTORY_SEPARATOR.$f;
                    if(is_dir($a)) $out[]=['name'=>$f,'path'=>rel($a),'ch'=>tree($a,$d+1)]; }
        ...
        case 'mkdir':
            $name=gp('name'); if(!$name) jErr('Name required');
            if(preg_match('/[\/\\\\<>:"\|?*\x00-\x1f]/',$name)) jErr('Invalid name');
            $base=gp('path')===''?ROOT_DIR:sp(gp('path'));
            $nd=$base.DIRECTORY_SEPARATOR.$name;
            if(file_exists($nd)) jErr('Already exists');
            if(!@mkdir($nd,0755)) jErr('Failed');
            jOk(['item'=>fi($nd)]);
        ...
        case 'touch':
            ...
            if(file_exists($nf)) jErr('Already exists');
            if(@file_put_contents($nf,'')===false) jErr('Failed');
            jOk(['item'=>fi($nf)]);

        case 'rename':
            $p=gp('path'); $n=gp('name');
            if(!$p||!$n) jErr('Path and name required');
            if(preg_match('/[\/\\\\<>:"\|?*\x00-\x1f]/',$n)) jErr('Invalid name');
            $a=sp($p); $na=dirname($a).DIRECTORY_SEPARATOR.$n;
            if(!file_exists($a)) jErr('Not found');
            if(file_exists($na)) jErr('Target exists');
            if(!@rename($a,$na)) jErr('Rename failed');
            jOk(['item'=>fi($na)]);

        case 'delete':
            $paths=$_POST['paths']??[];
            if(empty($paths)&&isset($_POST['path'])) $paths=[$_POST['path']];
            if(empty($paths)) jErr('No paths');
            $del=[];$err=[];
            foreach($paths as $p){
                try{
                    $a=sp($p);
                    if(!file_exists($a)){$err[]="$p: not found";continue;}
                    (is_dir($a)?rmRec($a):@unlink($a))?$del[]=$p:($err[]="$p: failed");
                }catch(Throwable $ex){$err[]=$p.': '.$ex->getMessage();}
            }
            if(!empty($err)&&empty($del)) jErr(implode('; ',$err));
            jOk(['del'=>$del,'err'=>$err]);

        case 'upload':
            $dest=gp('path')===''?ROOT_DIR:sp(gp('path'));
            if(!is_dir($dest)) jErr('Not a directory');
            if(empty($_FILES['files'])) jErr('No files');
            ...
            foreach($reindexed as $f){
                if($f['err']!==UPLOAD_ERR_OK){$err[]=$f['name'].': upload error';continue;}
                if($f['sz']>$max){$err[]=$f['name'].': too large';continue;}
                $n=preg_replace('/[^a-zA-Z0-9._\- ]/','_',$f['name']);
                $n=ltrim($n,'.');
                $d=$dest.DIRECTORY_SEPARATOR.$n;
                if(file_exists($d)){$pi=pathinfo($d);$d=$pi['dirname'].DIRECTORY_SEPARATOR.$pi['filename'].'_'.time().(isset($pi['extension'])?'.'.$pi['extension']:'');}
        ...
        case 'download':
            ...
            ob_clean(); flush(); readfile($a); exit;

        case 'read':
            $a=sp(gp('path'));
            if(!is_file($a)) jErr('Not a file');
            $c=file_get_contents($a);
            if($c===false) jErr('Cannot read');
            jOk(['content'=>$c,'name'=>basename($a)]);

        case 'write':
            $a=sp(gp('path'));
            if(!is_file($a)) jErr('Not a file');
            if(!is_writable($a)) jErr('Not writable');
            $c=$_POST['content']??'';
            if(file_put_contents($a,$c)===false) jErr('Write failed');
            jOk(['size'=>filesize($a)]);

        case 'zip':
            if(!class_exists('ZipArchive')) jErr('ZipArchive not available');
            $paths=$_POST['paths']??[];
            if(empty($paths)) jErr('No items');
            ...
            $z=new ZipArchive();
            if($z->open($zp,ZipArchive::CREATE|ZipArchive::OVERWRITE)!==true) jErr('Cannot create zip');
        ...
        case 'unzip':
            if(!class_exists('ZipArchive')) jErr('ZipArchive not available');
            $a=sp(gp('path'));
            if(!is_file($a)) jErr('Not found');
            if(strtolower(pathinfo($a,PATHINFO_EXTENSION))!=='zip') jErr('Not a zip');
            $dd=dirname($a).DIRECTORY_SEPARATOR.pathinfo($a,PATHINFO_FILENAME);
            if(!is_dir($dd)) @mkdir($dd,0755,true);
            $z=new ZipArchive();
            if($z->open($a)!==true) jErr('Cannot open zip');
            // zip-slip guard
            $realDd=realpath($dd);
            for($i=0;$i<$z->numFiles;$i++){
                $en=$z->getNameIndex($i);
                $ep=str_replace('\\','/',$realDd).'/'.str_replace('\\','/',$en);
                if(strpos($ep,str_replace('\\','/',$realDd).'/')!==0){$z->close();jErr('Zip-slip detected in: '.htmlspecialchars($en));}
            }
            $z->extractTo($dd); $z->close();
            jOk(['path'=>rel($dd)]);

        case 'search':
            $q=strtolower(gp('query')); if(!$q) jErr('Query required');
            $base=gp('path')===''?ROOT_DIR:sp(gp('path'));
            $results=[];
            function srch(string $dir,string $q,array &$out,int $d=0): void {
                ...
                foreach($e as $f){ if($f==='.'||$f==='..') continue;
                    $a=$dir.DIRECTORY_SEPARATOR.$f;
                    if(stripos($f,$q)!==false) $out[]=fi($a);
                    if(is_dir($a)) srch($a,$q,$out,$d+1); }
            }
            srch($base,$q,$results);
        ...
        case 'details':
            ...
            if(is_dir($a)){
                ...
                $cnt=0;$tsz=0; foreach($it as $f){$cnt++; if($f->isFile())$tsz+=$f->getSize();}
                $info['cnt']=$cnt; $info['tsz']=fmtSz($tsz);
            }
            jOk($info);

        default: jErr('Unknown action: '.htmlspecialchars($action));
        }
    } catch(Throwable $ex) { jErr('Server error: '.$ex->getMessage()); }
}
...
<div class="mo hid" id="moUpload">
  <div class="md">
    <div class="mh"><h3>Upload Files</h3><button class="mc" data-mo="moUpload">✕</button></div>
    <div class="mb">
      <div id="uplDrop" style="border: 2px dashed var(--bd); border-radius: var(--r); padding: 30px; text-align: center; margin-bottom: 15px; cursor: pointer;">
        <span style="font-size: 30px;">☁</span><br>
        <span style="color: var(--tx2);">Drag files here or click to browse</span>
      </div>
      <div id="uplList" style="max-height: 200px; overflow-y: auto; display: flex; flex-direction: column; gap: 5px;"></div>
    </div>
    <div class="mf">
      <button class="btn ghost sm" data-mo="moUpload">Cancel</button>
      <button class="btn pri sm" id="btnDoUpload">Upload All</button>
    </div>
  </div>
</div>
...
function ctxOpen(e, item) {
    ...
    ctx.querySelector('[data-a=edit]').style.display = !item.isDir ? '' : 'none';
...
const uplList = document.getElementById('uplList');
...
function renderStaged() {
    ...
    stagedFiles.forEach((f, i) => {
        const row = document.createElement('div');
        row.style = 'display: flex; align-items: center; justify-content: space-between; padding: 5px; background: var(--sf2); border-radius: 4px; font-size: 12px;';
        row.innerHTML = `
            <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 250px;">${esc(f.name)} (${fmtSz(f.size)})</span>
            <button class="mc" style="color: var(--rd);">✕</button>
        `;
        row.querySelector('button').onclick = () => { stagedFiles.splice(i, 1); renderStaged(); };
        uplList.appendChild(row);
    });
    btnDoUpload.disabled = stagedFiles.length === 0;
}

document.getElementById('btnUpload').onclick = () => openUploadDialog();
document.getElementById('uplDrop').onclick = () => fileInput.click();
fileInput.onchange = e => { if (e.target.files.length) { openUploadDialog(e.target.files); e.target.value = ''; } };
...
async function doUpload(files) {
    ...
    xhr.upload.addEventListener('progress', e => { 
        if (e.lengthComputable) showUpb(Math.round(e.loaded / e.total * 100), `Uploading… ${Math.round(e.loaded / e.total * 100)}%`); 
    });
...
fa.addEventListener('drop', e => { 
    S.dragN = 0; 
    dropOv.classList.add('hid'); 
    if (e.dataTransfer.files.length) openUploadDialog(e.dataTransfer.files); 
});
